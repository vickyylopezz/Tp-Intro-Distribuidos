...
class ClientArguments(Arguments.Arguments):
    ...
    def config(self):
        ...
        if self.operation == OperationEnum.UPLOAD:
            ...
            self.parser.add_argument('-s', '--src', type=str, default='./files/file.txt',
                                     required=False, help='source file path',
                                     dest='filepath')
        elif self.operation == OperationEnum.DOWNLOAD:
            default_name = './files/file.txt'
            self.parser.add_argument('-d', '--dst', type=str, default='new_file.txt',
                                     required=False, help='destination file path',
                                     dest='filepath')

        self.parser.add_argument('-n', '--name', type=str, default=default_name,
